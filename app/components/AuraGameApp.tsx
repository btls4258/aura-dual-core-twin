...
function CardChip({ card, selected, committed, disabled, onClick, onPreview }: { card: CardInstance; selected: boolean; committed?: boolean; disabled?: boolean; onClick: () => void; onPreview: () => void }) {
  return <button className={`card-chip ${card.type} ${selected ? "selected" : ""} ${committed ? "committed" : ""}`} data-card-instance={card.id} disabled={disabled} onClick={onClick} onDoubleClick={(event) => { event.preventDefault(); onPreview(); }} title={`${card.name} · 双击查看大图`} aria-label={`${card.name}，${card.functionName}，消耗 ${card.cost} 点能量`}>
    <img className="card-art" src={CARD_ART[card.type]} alt="" draggable={false} />
    <span className="card-chip-caption">{card.name}</span>
...
export function AuraGameApp() {
  ...
  return <main className="app-shell">
    {playedCard && <div key={playedCard.id} className={`played-card-effect ${playedCard.faction}`} style={{ "--card-origin-x": `${playedCard.origin.x}px`, "--card-origin-y": `${playedCard.origin.y}px` } as CSSProperties} onAnimationEnd={() => setPlayedCard((current) => current?.id === playedCard.id ? undefined : current)} aria-hidden="true"><img src={CARD_ART[playedCard.card.type]} alt="" /><span>{playedCard.card.name}</span></div>}
    {scanArrival && <div key={scanArrival.id} className={`scan-card-arrival ${scanArrival.faction}`} onAnimationEnd={() => setScanArrival((current) => current?.id === scanArrival.id ? undefined : current)} aria-hidden="true"><img src={CARD_ART[scanArrival.card.type]} alt="" /><span>识别成功 · {scanArrival.card.name}</span></div>}
    <header className="topbar">
      ...
      <div className="system-links"><span><i className="status-dot connected" />规则引擎</span><button className="robot-link-button" onClick={() => setRobotPanelOpen(true)}><i className={`status-dot ${robotMode === "live" ? "connected" : robotMode === "connecting" ? "waiting" : "mock"}`} />{robotLinkLabel}</button><button className="robot-debug-button" onClick={() => setRobotDebugOpen(true)}>机器人调试</button><button className="danger" onClick={emergencyStop}>紧急停止</button></div>
    </header>
    ...
    <div className="battle-layout">
      ...
      <section className="battle-center">
        ...
        {selected && <div className={`battle-action-slot ${agentBusy ? "judging" : "ready"}`}><img src={CARD_ART[selected.type]} alt="" /><div><small>{agentBusy ? `${activePersonality.name}正在裁决` : "待执行卡牌"}</small><b>{selected.name}</b><span>{selectedRange ? `${selected.type === "move" ? "移动上限" : "有效射程"} ${selectedRange} cm` : selected.functionName}</span></div></div>}
        {cardOutcome && <div key={cardOutcome.id} className={`card-outcome-fx ${cardOutcome.status} ${cardOutcome.faction}`} onAnimationEnd={() => setCardOutcome((current) => current?.id === cardOutcome.id ? undefined : current)} aria-hidden="true"><img src={CARD_ART[cardOutcome.card.type]} alt="" /><b>{cardOutcome.label}</b></div>}
    ...
    <section className="control-deck">
      ...
      <><div className={`command-grid ${intelOpen ? "" : "intel-collapsed"} ${tab !== "command" ? "tab-panel-hidden" : ""}`}>
        ...
        <div className="action-panel"><div className="section-title"><span>{decisionMode === "agent" ? "玩家提交的卡牌" : "玩家直接指令"}</span><small>{physicalSetup ? "等待实体牌登记" : selected ? `${selected.cost} 点能量` : activePlayer.auraHp <= 0 ? "AURA 失能" : "未选择"}</small></div>{activePlayer.auraHp <= 0 ? <div className="disabled-aura"><b>AURA 已失能</b><p>无法移动和出牌，只能由基地消耗生命完成重构。</p></div> : selected ? <div className="pending-card"><img className="pending-card-art" src={CARD_ART[selected.type]} alt="" onClick={() => setPreviewCard(selected)} /><div className="pending-card-copy"><div className="pending-card-title"><b>{selected.name}</b><span>{selected.functionName}</span><button className="preview-card-button" onClick={() => setPreviewCard(selected)}>查看大图</button></div><p>{selected.description}</p><code>{decisionMode === "agent" ? (selected.type === "move" ? "落点 / 路径由 Agent 决定" : "是否执行 / 目标由 Agent 决定") : (selected.type === "move" ? "点击地图选择落点" : "由玩家直接确认执行")}</code></div></div> : <div className="empty-action">{physicalSetup ? `请前往视觉链路继续登记${factionName(physicalSetup.faction)}手牌` : decisionMode === "agent" ? "玩家先选择卡牌，AURA 再决定如何执行" : "玩家选择卡牌并直接确认动作"}</div>}<div className="notice-line">{notice}</div><div className="primary-actions"><button className="secondary" disabled={agentBusy || activePlayer.auraHp <= 0 || Boolean(physicalSetup)} onClick={() => setGame((current) => ({ ...current, selectedCard: undefined, plannedTarget: undefined }))}>取消</button><button className="primary" disabled={!selected || agentBusy || activePlayer.auraHp <= 0 || Boolean(physicalSetup)} onClick={confirmAction}>{confirmLabel}</button><button className="end-turn" disabled={agentBusy || Boolean(physicalSetup)} onClick={endTurn}>结束回合</button>{activePlayer.auraHp === 0 && <button className="revive-action" disabled={activePlayer.revives <= 0 || activePlayer.baseHp <= 10} onClick={revive}>基地回血 · 消耗 10</button>}</div></div>
        <div className={`event-panel tactical-drawer ${intelOpen ? "open" : "collapsed"}`}><div className="section-title"><span>{decisionMode === "agent" ? "AURA 驾驶人格" : "玩家直控"}</span><div className="event-panel-tools">{decisionMode === "agent" && intelOpen && <select className="persona-select" value={personalities[game.active]} disabled={agentBusy} onChange={(event) => { const personality = event.target.value as PersonalityId; setPersonalities((current) => ({ ...current, [game.active]: personality })); setAgentResolution(null); }}>{Object.entries(PERSONALITIES).map(([id, personality]) => <option key={id} value={id}>{personality.name} · {personality.title}</option>)}</select>}<button className="drawer-toggle" onClick={() => setIntelOpen((open) => !open)} aria-label={intelOpen ? "收起人格和战报" : "展开人格和战报"}>{intelOpen ? "收起" : "战报"}</button></div></div>{decisionMode === "agent" ? <><div className="persona-card"><b>{activePersonality.name}</b><span>{activePersonality.description}</span><q>{activePersonality.credo}</q></div><div className={`agent-verdict ${agentBusy ? "thinking" : agentResolution?.decision ?? "idle"}`}><b>{agentBusy ? "正在裁决玩家指令…" : agentResolution?.label ?? "等待玩家提交卡牌"}</b>{agentResolution && !agentBusy && <><p>“{agentResolution.line}”</p><small>{agentResolution.reason} · {agentResolution.source === "deepseek" ? "DEEPSEEK" : "LOCAL"} {agentResolution.durationMs} ms</small></>}</div></> : <div className="manual-control-card"><b>完全服从</b><p>不经过人格裁决。移动落点由玩家点击，其他卡牌由玩家确认后立即执行。</p></div>}<div className="event-title"><span>战场事件</span><small>实时状态流</small></div><div className="event-list">{game.events.map((item) => <div key={item.id + item.time} className={item.kind}><time>{item.time}</time><p>{item.text}</p></div>)}</div></div>
    ...
    {previewCard && <div className="card-preview-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setPreviewCard(undefined); }}><section className="card-preview-modal" role="dialog" aria-modal="true" aria-label={`${previewCard.name}卡牌预览`}><button className="card-preview-close" onClick={() => setPreviewCard(undefined)} aria-label="关闭卡牌预览">×</button><img src={CARD_ART[previewCard.type]} alt={`${previewCard.name}卡面`} /><div><small>{previewCard.functionName} · {previewCard.cost} 点能量</small><h2>{previewCard.name}</h2><p>{previewCard.description}</p><button onClick={() => { selectCard(previewCard); setPreviewCard(undefined); }}>选择这张卡</button></div></section></div>}
